...
layout.tsx: Giao Diện Chung Bền Vững (Persistent Shell), Ko Bị Unmount Khi Chuyển Route
...
(group): Nhóm Các Route Lại Với Nhau Để Tiện Quản Lý Mà Ko Làm Thay Đổi Đường Dẫn URL
[id]: Tham Số Đường Dẫn Động (Dynamic Route Segment)
[...slug]: Khớp Tất Cả Các Đường Dẫn Phía Sau (Catch-All Route Segment)
[[...slug]]: Catch-All Nhưng Ko Bắt Buộc (Optional Catch-All)
...
cache: "no-store": Ko Lưu Cache, Luôn Lấy Dữ Liệu Mới Mỗi Lần Request (Tương Đương SSR - Server-Side Rendering)
...
Request Deduplication: Tự Động Gộp Các Request fetch Trùng URL Trong Cùng 1 Vòng Đời Request

"use server": Async Functions Chạy Trực Tiếp Trên Server & Có Thể Dc Gọi Từ Phía Client
No API Routes: Thực Hiện Gửi Dữ Liệu/Thao Tác DB Trực Tiếp Mà Ko Cần Định Nghĩa Endpoint API Riêng
...
dynamicParams = true (Mặc Định): Cho Phép Tự Động Sinh Trang Mới Ở Runtime Nếu ID Chưa Dc Pre-render Lúc Build
generateMetadata: Hàm Sinh Metadata SEO (Title, Description, OpenGraph) Động Tương Ứng Với Params Của Route

GET, POST, PUT, DELETE, PATCH: Các Hàm HTTP Method Dc Export Ra Để Xử Lý Request Tương Ứng
...
export const dynamic = "force-dynamic": Cấu Hình Ép Route API Luôn Luôn Chạy Động - Ko Lưu Cache
